...
export default function VenueEditForm({ id, onSuccess }: VenueEditFormProps) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setCapacity(String(data.capacity || '500'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Failed to update venue specifications');
      }
